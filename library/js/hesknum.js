 ...
 *
 */
function headcss(path) {
	var urlPathArray = path.split('/');
	var d = urlPathArray[urlPathArray.length - 1].split('.');d.pop();
	var l=d.length,e;
	if(l>1){
	 	e = d[1] + "_css";
	}else{
...
function headcss_remove(id) {
	jQuery("link[id=" + id + "_css]").remove();
}
function getElementClass(the_nth, selector) {
	var a = jQuery(selector).attr("class").split(" ");
	console.log("getElementClass : "+a)
	return a[the_nth];
}

if ( typeof jQuery != "undefined")
	jQuery(function($) {
		jQuery.fn.wp_remove_attr = function() {
			this.removeAttr('alt').removeAttr('title');
			return this;
		}
		jQuery.fn.wp_remove_dimension = function() {
		...
		/* demo => http://jsfiddle.net/sxGtM/3/
		 ...
		 */
		/*developing a cropped image based off its image height and have it to be fixed.*/
		jQuery.fn.adjustimg = function(ad_height, mode) {
			if (this.is("img")) {
				var wrap = this.parent(), w = this.width(), h = this.height(), scale = ad_height / h, ad_width = scale * w, delta = -1 * Math.abs(w - ad_width) / 2;
				this.wp_remove_attr().wp_remove_dimension().css('max-width', '1000px').css('height', parseInt(ad_height) + 'px').css('width', parseInt(ad_width) + 'px').css('left', parseInt(delta) + 'px').css('position', 'relative').css('max-height', '100%');
				wrap.css("overflow", "hidden").css("display", "block");
				return this;
			} else
				return this;
		}
		jQuery.fn.hasDesc = function(tagfunction) {
