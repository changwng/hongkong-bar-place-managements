...
(function($) {
	...
	var win = $(window), options, images, activeImage = -1, activeURL, prevImage, nextImage, compatibleOverlay, middle, centerWidth, centerHeight, ie6 = !window.XMLHttpRequest, hiddenElements = [], documentElement = document.documentElement,
	...
	*/
	...
	function animateCaption() {
		if(typeof(opencv) == "object"){
			var coords = $(image).faceDetection({background_image : true});
		}
		if (prevImage >= 0)
			$(prevLink).show();
		if (nextImage >= 0)
...
function scanSlimBox() {
	// AUTOLOAD CODE BLOCK (MAY BE CHANGED OR REMOVED)
	if ( typeof (wsm_base_obj) != "undefined") {
		var path = wsm_base_obj.jbin;
		//head.js(path + "opencv.js", path + "opencv_face.js", path + "jquery.facedetection.js", function() {
		//.faceDetection();
